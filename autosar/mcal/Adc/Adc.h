...
#define ADE_E_BUFFER_UNINIT               0x19u /*!< API called while result buffer pointer is not initialized. */
#define ADE_E_POWER_STATE_NOT_SUPPORTED   0x1Bu /*!< API call with unsupported power state request. */
#define ADC_E_PERIPHERAL_NOT_PREPARED     0x1Du /*!< ADC not prepared for requested target power state. */
#define ADC_E_BUSY                        0x0Bu /*!< API is called while another conversion is already running, a HW trigger is already enabled, a request is already stored in the queue. */
#define ADC_E_IDLE                        0x0Cu /*!< API is called while group is in state ADC_IDLE or non enabled group. */
#define ADC_E_NOT_DISENGAGED              0x1Au /*!< API called while one or more ADC groups are not in IDLE state. */
